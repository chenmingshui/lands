#!/usr/bin/env python
#script from Giovanni
from math import *
import os,string
from optparse import OptionParser
import ROOT
parser = OptionParser(usage="usage: %prog [options] work.tgz min max \nrun with --help to get list of options")
...
parser.add_option("-O", "--options",  dest="options",  default="--freq",  type="string", help="options to use for lands")
...
script.write("tar -czf old.tgz *txt *root\n");
script.write("rm -rf *txt *root\n");
script.write("tar -xzf {input}\n".format(input=datacards));
script.write("perl -i -p -e 's/..\/common/common/g' *txt\n");
for i,x in enumerate(points):
    seed = ("$((%d + $i))" % (i*10000)) if options.random == False else "0"
    ...
    if options.smart:
        if i < 0.25 * options.points:
            ...
            toys = "$(( 2 * $n ))";
    what = "--singlePoint %g " % x if options.signif == False else "--significance 1";
    #script.write("{cond} ./lands.exe -d *.txt -M Hybrid {opts} --nToysForCLsb {T2} --nToysForCLb {T} -v {v} -n {out}_s{seed}_r{x} --bNotCalcCLssbb --bSaveM2lnQ  -s {seed} -i {toys} {what} -L $CMSSW_BASE/lib/slc5_amd64_gcc434/libHiggsAnalysisCombinedLimit.so  --minuitSTRATEGY 0 --maximumFunctionCallsInAFit 500000 --bMultiSigProcShareSamePDF  \n".format(
    script.write("{cond} ./lands.exe -d *.txt -M Hybrid {opts} --nToysForCLsb {T2} --nToysForCLb {T} -v {v} -n {out}_s{seed}_r{x} --bNotCalcCLssbb --bSaveM2lnQ  -s {seed} -tH {T} {what} -L $CMSSW_BASE/lib/*/libHiggsAnalysisCombinedLimit.so  --minuitSTRATEGY 0 --maximumFunctionCallsInAFit 500000 --bMultiSigProcShareSamePDF  \n".format(
                opts=options.options, T2=2*options.T, T=options.T, seed=seed, out=options.out, what=what, v=options.v,
                cond=interleave, toys=toys,x=x
              ))

script.write("\n");
if options.signif == False: script.write("hadd {out}.root {out}*_m2lnQ2.root  \n".format( out=options.out))
else : script.write("hadd {out}.root *hybridSig_ts*.root \n".format( out=options.out))
script.write('echo "## Done at $(date)"\n');
script.write("tar -czf {out}_out.tgz *\n".format(out=options.out))
script.close()
os.system("chmod +x %s.sh" % options.out)

if not os.path.exists("lands.exe"):
    print "Creating a symlink to the lands.exe binary, need to make sure 'which lands.exe' exist"
    #os.system("cp -s $(which lands.exe) .")
    os.system("cp -s $CMSSW_BASE/src/UserCode/mschen/LandS/test/lands.exe .")
    #print " lands.exe is not found in current dir "#, os.system("pwd")
    #exit(1)
...
datasetpath = None
pset = None
#output_file = {out}.root,{out}_out.tgz
...
[USER]
script_exe = {out}.sh
additional_input_files = lands.exe,{input}
return_data = 1
""".format(input=datacards, out=options.out, sched=sched, srv=(1 if options.server else 0), queue=options.queue, jobs=options.j, total=options.t))
