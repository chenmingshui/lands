...
int main(int argc, const char* argv[]){
	...
	for(int np=0; np<nPoint; np++){
		for(int ns=0; ns<nSample; ns++ ) {
			...
			if(ns==3) nstmp=5;//zbb
			if(ns==4) nstmp=6;//tt
			EventArray[np][0][ns]= evt_4e[np][nstmp]	;//*0.2; //4e
			EventArray[np][1][ns]= evt_4m[np][nstmp]	;//*0.2; //4m
			EventArray[np][2][ns]= evt_2e2m[np][nstmp]	;//*0.2; //2e2m
		}
	}
	for(int np=0; np<nPoint; np++){
	...
	if(1){
		...
		lb.Bands(0.05, 1000, true, 100000, true, 10000 );
